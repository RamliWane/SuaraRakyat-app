      ...
      >
        {categories.map((c) => (
          <TouchableOpacity key={c.label} className="items-center bg-white rounded-xl px-3 py-3 border border-gray-200">
            <View className="w-14 h-14 rounded-xl items-center justify-center" style={{ backgroundColor: c.bg }}>
              <Ionicons name={c.icon as any} size={22} color={c.color} />
            </View>
            <Text className="text-[11px] font-medium text-gray-600 text-center mt-1">{c.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
    </View>
